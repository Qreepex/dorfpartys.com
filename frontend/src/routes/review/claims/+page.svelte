<script lang="ts">
	...
	let successMessage = $derived(
		form?.success ? (form.action === 'approve' ? 'Claim genehmigt ✓' : 'Claim abgelehnt') : null
	);
</script>

<svelte:head>
	<title>Event-Claims | Review | dorfpartys.com</title>
...
<div class="max-w-6xl">
	...
	{#if claims.length === 0}
		<p class="text-muted">Keine ausstehenden Claims.</p>
	{:else}
				...
				<tbody>
					{#each claims as claim (claim.id)}
						<tr class="border-b border-border hover:bg-bg-alt">
							<td class="px-4 py-3">{claim.eventTitle}</td>
							<td class="px-4 py-3">
