...
describe('resolve — kanonische URLs aus AGENTS.md 1.2 (kein Redirect)', () => {
	...
	it('bundesland/kreis/art', async () => {
		const result = await resolve('de', ['schleswig-holstein', 'ostholstein', 'schuetzenfeste'], repo);
		expect(result.kind).toBe('result');
	});

	it('bundesland/kreis/monat', async () => {
		const result = await resolve('de', ['schleswig-holstein', 'ostholstein', 'august'], repo);
		expect(result.kind).toBe('result');
	});

	it('bundesland/kreis', async () => {
		const result = await resolve('de', ['schleswig-holstein', 'ostholstein'], repo);
		expect(result.kind).toBe('result');
	});
	...
	it('bundesland/art', async () => {
		const result = await resolve('de', ['schleswig-holstein', 'schuetzenfeste'], repo);
		expect(result.kind).toBe('result');
	});
...
describe('resolve — Kanonisierung/Redirects (AGENTS.md 1.4)', () => {
	...
	it('mehr als 4 Segmente ist 404', async () => {
		const result = await resolve(
			'de',
			['schleswig-holstein', 'ostholstein', 'schuetzenfeste', 'august', 'zuviel'],
			repo
		);
		expect(result).toEqual({ kind: 'not-found' });
	});
