...
import { MONTHS, buildCountryRootUrl, buildFilterUrl, resolverInputSchema } from '@dorfpartys/shared';
...
async function buildBreadcrumbsForResult(
	...
	filters: { bundeslandSlug: string | null; kreisSlug: string | null; artSlug: string | null; monatSlug: string | null }
) {
	const items = [{ name: country.toUpperCase(), url: buildCountryRootUrl(country) }];
	...
	if (filters.kreisSlug) {
		const [row] = await db.select({ name: kreis.name }).from(kreis).where(eq(kreis.slug, filters.kreisSlug));
		items.push({
			name: row?.name ?? filters.kreisSlug,
			url: buildFilterUrl(country, { bundeslandSlug: filters.bundeslandSlug, kreisSlug: filters.kreisSlug })
		});
	}

	if (filters.artSlug) {
		const [row] = await db.select({ name: partyArt.name }).from(partyArt).where(eq(partyArt.slug, filters.artSlug));
		items.push({
...
export const resolverRouter = router({
	resolve: publicProcedure.input(resolverInputSchema).query(async ({ ctx, input }) => {
		...
		const breadcrumbJsonLd = await buildBreadcrumbsForResult(ctx.db, input.country, outcome.filters);
