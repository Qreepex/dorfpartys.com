...
const S3_ENDPOINT = process.env.S3_ENDPOINT || "https://speicher.dorfpartys.com";
...
async function generateOgImage(
	...
	const displaySubtitle = subtitle.length > 65 ? subtitle.slice(0, 62) + "…" : subtitle;
    ...
    <svg width="${IMAGE_WIDTH}" height="${IMAGE_HEIGHT}" xmlns="http://www.w3.org/2000/svg">
      ...
      ${confettiElements
			.map(
				(el) => `<rect x="${el.x - el.size / 2}" y="${el.y - el.size / 2}" width="${el.size}" height="${el.size}" fill="${el.color}" opacity="0.8" transform="rotate(${el.rotate} ${el.x} ${el.y})"/>`,
			)
			.join("")}
      ...
      ${confettiBottomElements
			.map(
				(el) => `<circle cx="${el.x}" cy="${el.y}" r="${el.size}" fill="${el.color}" opacity="0.6"/>`,
			)
			.join("")}
    </svg>
  `;
...
> {
	...
	try {
		...
		const partyArts = await sql`SELECT slug, name FROM party_art WHERE active = true`;
		const bundeslaender = await sql`SELECT id, slug, name, country FROM bundesland`;
...
async function generateAllOgImages() {
	...
	for (const combo of imagesToProcess) {
		try {
			...
			console.error(`   ❌ Fehler bei ${combo.title}:`, err instanceof Error ? err.message : err);
			failCount++;
		}
	}
