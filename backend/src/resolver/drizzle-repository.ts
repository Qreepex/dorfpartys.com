...
import { bundesland, event, eventPhoto, kreis, partyArt } from '../db/schema.js';
...
function whereConditions(country: Country, filters: EventFilterIds) {
	...
	if (filters.bundeslandId) conditions.push(eq(event.bundeslandId, filters.bundeslandId));
	if (filters.kreisId) conditions.push(eq(event.kreisId, filters.kreisId));
	if (filters.partyArtId) conditions.push(eq(event.partyArtId, filters.partyArtId));
	if (filters.monatNumber) {
		conditions.push(sql`extract(month from ${event.startDate}) = ${filters.monatNumber}`);
	}
	return and(...conditions);
}

export function createDrizzleTaxonomyRepository(db: Database): TaxonomyRepository {
	return {
		...
		async listApprovedEvents(country, filters, limit = 50): Promise<EventListItem[]> {
			const rows = await db
				.select({
					slug: event.slug,
					title: event.title,
