 ...
 */
export async function resolveUserFromClaims(db: Database, claims: AuthentikClaims) {
...
export async function findUserByAuthentikSubject(db: Database, subject: string) {
	const [row] = await db.select().from(user).where(eq(user.authentikSubject, subject)).limit(1);
	return row;
}
